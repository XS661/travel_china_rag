...
2. **知识检索**：BM25 算法匹配最相关的 5 条知识片段，优先匹配目标城市
...
| **中文分词** | jieba                             | 分词 + 关键词提取             |
| **检索算法** | rank-bm25（默认） / 倒排索引      | 两种方案可切换对比            |
...
| 对比维度   | 关键词匹配                  | BM25                                     |
| ---------- | --------------------------- | ---------------------------------------- |
| 原理       | jieba 分词 + 倒排索引求交集 | 词频(TF) × 逆文档频率(IDF) + 长度归一化 |
| 实现复杂度 | 低                          | 中                                       |
| 检索速度   | 极快                        | 快                                       |
| 长文本匹配 | 一般                        | 较好（长度归一化）                       |
| 可解释性   | 强（命中词可见）            | 中（分数可查）                           |
...
默认使用 BM25，前端页面可切换检索方式，也可通过 API 参数切换。
